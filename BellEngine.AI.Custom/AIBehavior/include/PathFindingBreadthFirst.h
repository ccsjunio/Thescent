...
class PathFindingBreadthFirst : public IPathFinding
{
private:
    Graph* graph;
    Graph::Node* rootNode;
    bool isActive, isUpdateComplete, goalFound;
    std::queue<Graph::Node*> openQueue;
    ...
    const bool OrderByClosestToRoot(std::vector<Graph::Node*>& path);
public:
    PathFindingBreadthFirst()
    {
    ...
    void Activate(const bool& option);
    const bool IsActive();
