...
#include <glm/mat4x4.hpp> // Matrix4x4
...
const std::string DebugRenderer::DEFAULT_GEOM_SHADER_SOURCE_LINES = "\
        #version 400                                     \n \
                                                         \n \
        layout(lines)               in;                  \n \
...
// Default geometry shader is a pass through
const std::string DebugRenderer::DEFAULT_GEOM_SHADER_SOURCE_TRIANGLES = "\
        #version 400                                     \n \
                                                         \n \
        layout(triangles)               in;              \n \
...
void DebugRenderer::m_parseStringIntoMultiLine( std::string singleLineSource, std::vector<std::string> &vecMultiline )
{
#ifdef _WIN32
...
bool DebugRenderer::m_compileShaderFromSource( unsigned int &shaderID, std::vector<std::string> &vecMultilineSource, std::string &errorText )
{
	errorText = "";
...
bool DebugRenderer::m_wasThereALinkError( unsigned int programID, std::string &error )
{
	error = "";	// No error
...
bool DebugRenderer::initialize(void)
{
...
//                        |___/                                    
	...
	if ( ! this->resizeBufferForMeshes(DebugRenderer::DEFAULT_MESHBUFFERSIZE_PADDING_RATIO) )
	{
		return false;
	}
...
//static
const float DebugRenderer::DEFAULT_MESHBUFFERSIZE_PADDING_RATIO = 1.10f; //or 10 %

bool DebugRenderer::resizeBufferForMeshes(float meshBufferPaddingRatio /*=DEFAULT_MESHBUFFERSIZE_PADDING_RATIO*/)
{
	//TODO
...
DebugRenderer::DebugRenderer()
{
	...
	this->m_geometryShaderSource_Lines = DebugRenderer::DEFAULT_GEOM_SHADER_SOURCE_LINES;
	this->m_geometryShaderSource_Triangles = DebugRenderer::DEFAULT_GEOM_SHADER_SOURCE_TRIANGLES;
	...
	this->m_pGLRenderState = new GLRenderStateHelper();
...
void DebugRenderer::RenderDebugObjects(const Matrix4D &matCameraView, const Matrix4D &matProjection, double deltaTime)
{
	Matrix4D matView = matCameraView;
	Matrix4D matProj = matProjection;
...
DebugRenderer::sGLDrawState::sGLDrawState()
{
...
void DebugRenderer::m_SaveGLState( sGLDrawState &curGLState )
{
...
void DebugRenderer::m_RestoreGLState( const sGLDrawState &curGLState )
{
...
void DebugRenderer::m_RenderDebugTriangles(const Matrix4D &matCameraView, const Matrix4D &matProjection, double deltaTime)
{
	...
	glUniformMatrix4fv( this->m_TriangleShader_matModelUniformLoc, 1, GL_FALSE, 
					(const GLfloat*) glm::value_ptr(Matrix4D(1.0f)) );
...
void DebugRenderer::m_RenderDebugLines(const Matrix4D &matCameraView, const Matrix4D &matProjection, double deltaTime)
{
	// Get current GL state:
	sGLDrawState curGLState;
	this->m_SaveGLState( curGLState );
...
void DebugRenderer::m_RenderDebugPoints(const Matrix4D &matCameraView, const Matrix4D &matProjection, double deltaTime)
{
	// Get current GL state:
	sGLDrawState curGLState;
	this->m_SaveGLState( curGLState );
...
void DebugRenderer::m_copyLinesIntoRenderBuffer(double deltaTime)
{
	// Used to keep the "persistent" ones...
...
void DebugRenderer::m_copyPointsIntoRenderBuffer(double deltaTime)
{
	// TODO: 
...
void DebugRenderer::addTriangle(Vector3D v1XYZ, Vector3D v2XYZ, Vector3D v3XYZ, Vector3D colour, float lifeTime/*=0.0f*/)
{
...
void DebugRenderer::addLine(Vector3D startXYZ, Vector3D endXYZ, Vector3D colour, float lifeTime/*=0.0f*/)
{
...
void DebugRenderer::addPoint(Vector3D xyz, Vector3D colour, float lifeTime/*=0.0f*/, float pointSize/*=1.0f*/)
{
...
void DebugRenderer::addDebugSphere(Vector3D xyz, Vector3D colour, float scale, float lifeTime/*=0.0f*/)
{
...
void DebugRenderer::addDebugMesh(std::string meshName, Vector3D xyz, Vector3D orientXYZ_Euler, Vector3D colour, float scale, float lifeTime/*=0.0f*/)
{
...
void DebugRenderer::addDebugMesh(std::string meshName, Vector3D xyz, glm::quat orientXYZ_Quaternion, Vector3D colour, float scale, float lifeTime/*=0.0f*/)
{
	iDebugRenderer::sDebugMesh debugMesh;
	debugMesh.name = meshName;
	debugMesh.scale = scale;
...
std::string DebugRenderer::getLastError(bool bClearError /*=true*/)
{
...
{
	this->v[0] = Vector3D(0.0f); this->v[1] = Vector3D(0.0f); this->v[2] = Vector3D(0.0f);
	this->colour = Vector3D(1.0f);	// white
	this->lifeTime = 0.0f;
	this->bIgnorDepthBufferOnDraw = true;
	return;
}

iDebugRenderer::sDebugTri::sDebugTri(Vector3D v1, Vector3D v2, Vector3D v3)
{
	this->v[0] = v1;
	this->v[1] = v2;
	this->v[2] = v3;
	this->colour = Vector3D(1.0f,1.0f,1.0f);
	this->lifeTime = 1.0f;
	this->bIgnorDepthBufferOnDraw = true;
	return;
}

iDebugRenderer::sDebugTri::sDebugTri(Vector3D v1, Vector3D v2, Vector3D v3, Vector3D colour, float lifeTime/*=0.0f*/)
{
	this->v[0] = v1;
	this->v[1] = v2;
	this->v[2] = v3;
...
iDebugRenderer::sDebugTri::sDebugTri(Vector3D v[3], Vector3D colour, float lifeTime/*=0.0f*/)
{
	this->v[0] = v[0];
	this->v[1] = v[1];
	this->v[2] = v[2];
...
iDebugRenderer::sDebugLine::sDebugLine(Vector3D start, Vector3D end)
{
	this->points[0] = start;
	this->points[1] = end;
	this->colour = Vector3D(1.0f,1.0f,1.0f);
...
iDebugRenderer::sDebugLine::sDebugLine(Vector3D start, Vector3D end, Vector3D colour, float lifeTime/*=0.0f*/)
{
	this->points[0] = start;
	this->points[1] = end;
	this->colour = colour;
...
iDebugRenderer::sDebugLine::sDebugLine(Vector3D points[2], Vector3D colour, float lifeTime/*=0.0f*/)
{
...
iDebugRenderer::sDebugPoint::sDebugPoint(Vector3D xyz)		// colour=1,1,1		lifeTime=0.0f	pointSize=1.0f
{
	this->xyz = xyz;
	this->colour = Vector3D(1.0f,1.0f,1.0f);
	this->pointSize = 1.0f;
...
iDebugRenderer::sDebugPoint::sDebugPoint(Vector3D xyz, Vector3D colour, float lifeTime/*=0.0f*/, float pointSize/*=1.0f*/)
{
	this->xyz = xyz;
	this->colour = colour;
...
iDebugRenderer::sDebugMesh::sDebugMesh()
{
	...
	this->xyz = Vector3D(0.0f,0.0f,0.0f);
	this->qOrientation = glm::quat(Vector3D(0.0f,0.0f,0.0f));
	this->scale = 1.0f;
	this->colour = Vector3D(1.0f,1.0f,1.0f);
	this->lifeTime = 0.0f;
	this->bIgnorDepthBufferOnDraw = true;
...
iDebugRenderer::sDebugMesh::sDebugMesh(std::string name, Vector3D xyz, float lifeTime/*=0.0f*/)
{
	this->name = name;
	this->xyz = xyz;
	this->lifeTime = lifeTime;
...
iDebugRenderer::sDebugMesh::sDebugMesh(std::string name, Vector3D xyz, Vector3D colour, float scale, float lifeTime/*=0.0f*/)
{
	this->name = name;
	this->xyz = xyz;
	this->lifeTime = lifeTime;
...
iDebugRenderer::sDebugMesh::sDebugMesh(std::string name, Vector3D xyz, Vector3D EulerOrientation, Vector3D colour, 
...
iDebugRenderer::sDebugMesh::sDebugMesh(std::string name, Vector3D xyz, glm::quat qOrientation, Vector3D colour, 
                                       float scale, float lifeTime/*=0.0f*/)
{
	this->name = name;
	this->xyz = xyz;
