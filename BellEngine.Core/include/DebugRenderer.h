...
class GLRenderStateHelper;	// Forward declare so we don't have to include the GL headers
...
public:
	...
	// Renders scene
	void RenderDebugObjects(const Matrix4D &matCameraView, const Matrix4D &matProjection, double deltaTime);
private:
	void m_RenderDebugTriangles(const Matrix4D &matCameraView, const Matrix4D &matProjection, double deltaTime);
	void m_RenderDebugLines(const Matrix4D &matCameraView, const Matrix4D &matProjection, double deltaTime);
	void m_RenderDebugPoints(const Matrix4D &matCameraView, const Matrix4D &matProjection, double deltaTime);
	void m_RenderDebugMeshes(const Matrix4D &matCameraView, const Matrix4D &matProjection, double deltaTime);

	// Used to save and restore the render state during the debug render calls
	GLRenderStateHelper* m_pGLRenderState;
...
public:
	...
	virtual void addTriangle(Vector3D v1XYZ, Vector3D v2XYZ, Vector3D v3XYZ, Vector3D colour, float lifeTime=0.0f);
	virtual void addTriangle(drTri &tri);
	virtual void addLine(Vector3D startXYZ, Vector3D endXYZ, Vector3D colour, float lifeTime=0.0f);
	virtual void addLine(drLine &line);
	virtual void addPoint(Vector3D xyz, Vector3D colour, float lifeTime=0.0f, float pointSize=1.0f);
	virtual void addPoint(drPoint &point);

	// Replaces the DrawDebugSphere
	virtual void addDebugSphere(Vector3D xyz, Vector3D colour, float scale, float lifeTime=0.0f);
	virtual void addDebugMesh(std::string meshName, Vector3D xyz, Vector3D orientXYZ_Euler, Vector3D colour, float scale, float lifeTime=0.0f);
	virtual void addDebugMesh(std::string meshName, Vector3D xyz, glm::quat orientXYZ_Quaternion, Vector3D colour, float scale, float lifeTime=0.0f);
