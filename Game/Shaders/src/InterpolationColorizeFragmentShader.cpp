#include "..\include\InterpolationColorizeFragmentShader.h"
#include <BellEngineID.h>
#include <LightPoint.h>
#include <LightDirectional.h>
#include <LightSpot.h>

const bool InterpolationColorizeFragmentShader::GetUniformLocations()
{
	this->colorA_ul = glGetUniformLocation(this->programID, "interpolatinColorize.colorA");
	this->colorB_ul = glGetUniformLocation(this->programID, "interpolatinColorize.colorB");
	this->specularColour_UL = glGetUniformLocation(this->programID, "specularColor");
	this->deltaTime_ul = glGetUniformLocation(this->programID, "timeInfo.deltaTime");	
	this->runTime_ul = glGetUniformLocation(this->programID, "timeInfo.runTime");
...
InterpolationColorizeFragmentShader::InterpolationColorizeFragmentShader()
{
	this->programID = 
		this->deltaTime_ul =
		this->runTime_ul =
...
const unsigned long long InterpolationColorizeFragmentShader::GetTypeID()
{
	return ENGINE_OBJ_ID_GPH_GLSHADER_FRAGMENT_COLORIZE;
}

const std::string InterpolationColorizeFragmentShader::GetType()
{
	return "InterpolationColorizeFragmentShader";
}

const std::string InterpolationColorizeFragmentShader::GetName()
{
	return this->name;
}

void InterpolationColorizeFragmentShader::SetName(const std::string& name)
{
	this->name = name;
}

const bool InterpolationColorizeFragmentShader::Initialize()
{
	bool rc = true;
...
void InterpolationColorizeFragmentShader::SetProgramID(const unsigned int& programID)
{
...
const bool InterpolationColorizeFragmentShader::SetLights(const std::vector<ILight*>& lights)
{
	...
	for (size_t i = 0; i < current_light_count; i++)
	{
		...
		default:
		{
			printf("InterpolationColorizeFragmentShader: Passing unknown light to the shader\n!\n");
		}
		break;
		}
...
const bool InterpolationColorizeFragmentShader::SetDeltaTime(const double& value)
{
	glUniform1f(this->deltaTime_ul, (GLfloat)value);
	return true;
}

const bool InterpolationColorizeFragmentShader::SetRunTime(const double& value)
{
	glUniform1f(this->runTime_ul, (GLfloat)value);
	return true;
}

const bool InterpolationColorizeFragmentShader::SetColorA(const Vector4D& value)
{
	glUniform4f(this->colorA_ul, value.r, value.g, value.b, value.a);
	return true;
}

const bool InterpolationColorizeFragmentShader::SetColorB(const Vector4D& value)
{
	glUniform4f(this->colorB_ul, value.r, value.g, value.b, value.a);
	return true;
}

const bool InterpolationColorizeFragmentShader::SetSpecular(const Vector4D& value)
{
